...
class NodeHandleSeq[D, U, B <: Data](val seq: Seq[NodeHandle[D, U, B, D, U, B]])
{
  require (!seq.isEmpty, "NodeHandleSeq must be non-empty; otherwise, arity information is lost when composed. Suggested work-around: include an IdentityNode")
  // connecting an outward node through a Seq of adapters
  def := (h: OutwardNodeHandle[D, U, B])(implicit p: Parameters, sourceInfo: SourceInfo): OutwardNodeHandle[D, U, B] = seq.reduce(_ :=  _) :=  h
  def :*=(h: OutwardNodeHandle[D, U, B])(implicit p: Parameters, sourceInfo: SourceInfo): OutwardNodeHandle[D, U, B] = seq.reduce(_ :*= _) :*= h
  def :=*(h: OutwardNodeHandle[D, U, B])(implicit p: Parameters, sourceInfo: SourceInfo): OutwardNodeHandle[D, U, B] = seq.reduce(_ :=* _) :=* h
  def :=?(h: OutwardNodeHandle[D, U, B])(implicit p: Parameters, sourceInfo: SourceInfo): OutwardNodeHandle[D, U, B] = seq.reduce(_ :=? _) :=? h
}
...
trait InwardNodeHandle[DI, UI, BI <: Data] extends NoHandle
{
  ...
  // connecting an input node to a Seq of adapters
  def := (seq: NodeHandleSeq[DI, UI, BI])(implicit p: Parameters, sourceInfo: SourceInfo): InwardNodeHandle[DI, UI, BI] = this :=  seq.seq.reduce(_ :=  _)
  def :*=(seq: NodeHandleSeq[DI, UI, BI])(implicit p: Parameters, sourceInfo: SourceInfo): InwardNodeHandle[DI, UI, BI] = this :*= seq.seq.reduce(_ :*= _)
  def :=*(seq: NodeHandleSeq[DI, UI, BI])(implicit p: Parameters, sourceInfo: SourceInfo): InwardNodeHandle[DI, UI, BI] = this :=* seq.seq.reduce(_ :=* _)
  def :=?(seq: NodeHandleSeq[DI, UI, BI])(implicit p: Parameters, sourceInfo: SourceInfo): InwardNodeHandle[DI, UI, BI] = this :=? seq.seq.reduce(_ :=? _)
}
