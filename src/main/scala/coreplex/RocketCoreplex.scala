...
case class TileMasterPortParams(
  ...
  def adapt(coreplex: HasPeripheryBus)
    ...
    (Seq(tile_master_buffer.node, tile_master_fixer.node)
     ++ tile_master_blocker.map(_.node) ++ tile_master_cork.map(_.node)) :=* masterNode
  }
}
...
case class TileSlavePortParams(
  ...
  def adapt(coreplex: HasPeripheryBus)
    ...
    slaveNode :*= (Seq(tile_slave_buffer.node) ++ tile_slave_blocker.map(_.node))
  }
}
